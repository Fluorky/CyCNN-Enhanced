...
with open("train_test_scenarios_GTSRB_RGB.json") as f:
    train_test_dict = json.load(f)

main_script = "main.py"
venv_python = "venv/bin/python"

base_data_dir = "./data/GTSRB_RGB" 
merged_dir = base_data_dir
dataset_GTSRB_RGB_non_rotated = os.path.join(base_data_dir, "dataset_GTSRB_RGB_non_rotated")

base_save_dir = "./saves/GTSRB_RGB/"
base_log_dir = "./logs/json_GTSRB_RGB/"
...
def generate_model_save_path(model_name, polar_transform, train_set):
    fname = f"GTSRB-RGB-custom-{model_name}-{polar_transform}_{train_set}.pt"
    ...
    for model_name in models:
        for polar_transform in polar_transforms:
            for train_set, test_sets in train_test_dict.items():
                ...
                train_log_file = os.path.join(train_log_dir, f"GTSRB-RGB-custom-{model_name}-{polar_transform}_{train_set_safe}_train.txt")
                ...
                for test_set in test_sets:
                    ...
                    test_subdir = os.path.join(test_log_dir, f"GTSRB-RGB-custom-{model_name}-{polar_transform}_{train_set_safe}")
                    cm_output_dir = os.path.join(cm_log_dir, f"GTSRB-RGB-custom-{model_name}-{polar_transform}_{train_set_safe}/{train_set_safe}_test_on_{test_set_safe}")
                    os.makedirs(test_subdir, exist_ok=True)
                    test_log_file = os.path.join(test_subdir, f"GTSRB-RGB-custom-{model_name}-{polar_transform}_{train_set_safe}_test_on_{test_set_safe}.txt")

                    print(f"--- TESTING {train_set} model on {test_set} with {model_name} and {polar_transform} ---")

                    test_cmd = (
                        f"{venv_python} {main_script} "
                        f"--test --model={model_name} --dataset=GTSRB-RGB-custom "
                        ...
                        # f"--train-set={train_set_safe} --test-set={test_set_safe}"
                    )
